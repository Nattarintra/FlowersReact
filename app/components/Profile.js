...
function Profile() {
  ...
  const [state, setState] = useImmer({
    ...
    profileData: {
      ...
      isFollowing: false,
      counts: { postCount: "", followingCount: "" }
    }
  })
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const response = await Axios.post(`/profile/${username}`, { token: appState.user.token }, { cancelToken: flowersRequest.token })
  ...
  useEffect(() => {
    if (state.startFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const response = await Axios.post(`/addFollow/${state.profileData.profileUsername}`, { token: appState.user.token }, { cancelToken: flowersRequest.token })
          //console.log(response.data)
          setState(draft => {
  ...
  useEffect(() => {
    if (state.stopFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const response = await Axios.post(`/removeFollow/${state.profileData.profileUsername}`, { token: appState.user.token }, { cancelToken: flowersRequest.token })
          //console.log(response.data)
          setState(draft => {
