...
function EditPost(props) {
  ...
  const [state, dispatch] = useImmerReducer(flowersReducer, originalState)
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        const response = await Axios.get(`/post/${state.id}`, { cancelToken: flowersRequest.token })
        if (response.data) {
  ...
  useEffect(() => {
    if (state.sendCount) {
      ...
      async function fetchPost() {
        try {
          const response = await Axios.post(`/post/${state.id}/edit`, { title: state.title.value, body: state.body.value, token: appState.user.token }, { cancelToken: flowersRequest.token })
          //dispatch({ type: "fetchComplete", value: response.data })
